...
function parseURL(req_url) {
  ...
  if (!match[1]) {
    if (/^https?:/i.test(req_url)) {
      ...
      req_url = addSlashToReqUrl(req_url);
    }
...
// removes the protocol from the req_url. Firebase removes a slash which triggers "if (/^https?:/i.test(req_url))" in parseURL(req_url).
// This only fixes https protocols. http will fail.
function addSlashToReqUrl(request_url) {
  var protocolSlice = request_url.slice(0, 8);

  if (protocolSlice !== 'https://') {
    return request_url.slice(7, request_url.length);
  }
